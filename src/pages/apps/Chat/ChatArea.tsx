...
const ChatArea = ({ selectedUser }: ChatAreaProps) => {
   ...
   * form methods
   */
  const methods = useForm<{ newMessage: string }>({ resolver: schemaResolver }); // Specify the form fields explicitly
  const {
    handleSubmit,
    register,
    control,
   ...
   */
  const sendChatMessage: SubmitHandler<{ newMessage: string }> = async (data) => {
    let newUserMessages = [...userMessages];
    newUserMessages.push({
      id: userMessages.length + 1,
      from: toUser,
      to: selectedUser,
      message: { type: "text", value: data.newMessage }, // Corrected here
    ...
    setTimeout(() => {
      ...
      otherNewMessages.push({
        ...
        from: selectedUser,
        to: toUser,
        message: { type: "text", value: data.newMessage }, // And here
        sendOn: new Date().getHours() + ":" + new Date().getMinutes(),
      })
