      ...
      <div className="text-center my-5">
        <Card>
          <Card.Body>
            ...
            <p className="text-muted">There are no kitchens in the system yet.</p>
    ...
    <>
      ...
      {searchTerm && (!filteredMenuItems || filteredMenuItems.length === 0) ? (
        <div className="text-center my-5">
          <Card>
            <Card.Body>
              <i className="mdi mdi-file-search-outline text-muted" style={{ fontSize: "48px" }}></i>
              <h4 className="mt-3">No Results Found</h4>
              <p className="text-muted">No kitchens match your search criteria "{searchTerm}".</p>
            </Card.Body>
          </Card>
        </div>
      ) : (
                ...
                <Card className="product-box h-100">
                  <Card.Body className="d-flex flex-column">
                    <div className="bg-light mb-3">
                      <img src={item?.kitchen_image} alt={item?.kitchen_name} className="img-fluid" style={{ width: "100%", height: "200px", objectFit: "cover" }} />
                    </div>
                    <div className="product-info mt-auto">
                      <h5 className="font-16 mt-0">
                        <Link to={`/apps/kitchen/${item._id}`} className="text-dark">{item?.kitchen_name}</Link>
                      </h5>
