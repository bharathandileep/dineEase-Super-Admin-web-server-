...
import { ChevronLeft, ChevronRight, MapPin, Upload, Check, X, Clock } from 'lucide-react';
...
const KitchenRegistrationForm = () => {
  ...
  const handleTimingChange = (index: number, field: keyof KitchenTiming, value: string | boolean) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: keyof FormData) => {
    ...
    if (field === 'kitchenImages') {
      ...
      setFormData(prev => ({ ...prev, kitchenImages: [...prev.kitchenImages, ...newFiles] }));
      ...
      newFiles.forEach(file => {
        ...
        reader.onloadend = () => {
          setPreviews(prev => ({
            ...prev,
            kitchenImages: [...(prev.kitchenImages as string[] || []), reader.result as string]
          }));
        };
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, field: keyof FormData) => {
    e.preventDefault();
  ...
  const removeFile = (field: keyof FormData, index?: number) => {
    if (field === 'kitchenImages' && typeof index === 'number') {
      ...
      setPreviews(prev => ({
        ...prev,
        kitchenImages: (prev.kitchenImages as string[]).filter((_, i) => i !== index)
      }));
    } else {
...
function transformData(inputData: any):any {
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    await createKitchen(transformData(formData));
        ...
        <label htmlFor={field} className="cursor-pointer">
          {multiple ? (
            <div className="grid grid-cols-2 gap-4 mt-4">
              {(previews.kitchenImages as string[] || []).map((preview, index) => (
              ...
              {(!previews.kitchenImages || (previews.kitchenImages as string[]).length === 0) && (
  ...
  const inputClasses = "mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border";
...
></div>
        ...
        <div className="d-flex justify-content-between">
          <div className={`d-flex align-items-center ${currentStep >= 1 ? 'text-indigo' : 'text-muted'}`}>
            <span className="badge rounded-circle border">{currentStep > 1 ? <Check /> : '1'}</span>
            <span className="ms-2">Basic Details</span>
          </div>
          <div className={`d-flex align-items-center ${currentStep >= 2 ? 'text-indigo' : 'text-muted'}`}>
            <span className="badge rounded-circle border">{currentStep > 2 ? <Check /> : '2'}</span>
            <span className="ms-2">Documents</span>
          </div>
          <div className={`d-flex align-items-center ${currentStep >= 3 ? 'text-indigo' : 'text-muted'}`}>
            <span className="badge rounded-circle border">{currentStep > 3 ? <Check /> : '3'}</span>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-4 rounded shadow">
        {currentStep === 1 && (
          <div>
            ...
            {/* Address Details */}
            <h3 className="fs-5 fw-medium text-gray-900">Address Details</h3>
            <div className="row">
          ...
          <div>
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-12">
                <FileUploadField label="PAN Card Image" field="panImage" file={formData.panImage} />
              </div>
            </div>
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-12">
                <FileUploadField label="GST Certificate Image" field="gstImage" file={formData.gstImage} />
              </div>
            </div>
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-12">
                <FileUploadField label="FSSAI Certificate Image" field="fssaiImage" file={formData.fssaiImage} />
              </div>
            </div>
          </div>
        )}
